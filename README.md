...
| **エクスポート**   | スケジュールをJSONで出力               | 🔄 実装中  |
| **インポート**     | JSONファイルからスケジュールを読み込み | 🔄 実装中  |
| **出発時間管理**   | 移動時間を考慮した出発時間の自動表示   | 🔄 実装中  |
...
| **データ共有機能**       | [担当者名を記入] | [GitHubアカウントを記入] | エクスポート・インポート、スケジュール共有 (`export/`, `utils/data_io.py`) |
| **出発時間管理** | [担当者名を記入] | [GitHubアカウントを記入] | 移動時間計算、ユーザー設定管理 (`front_end/settings.py`)             |
